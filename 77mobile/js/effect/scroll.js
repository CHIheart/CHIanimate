...
oCallbacks，回调函数列表对象，可以使用的属性有
	.init(oJQcontainer,oJQkid)，完成初始化时执行的函数，参数依次为容器、子层的JQ对象
	.move(oJQcontainer,oJQkid)，每一次滚动时执行的函数，参数依次为容器、子层的JQ对象
如果生成有名对象的话，可以调用的方法有
	.on()，开始滚动，使用后，auto被设置为true
	.off()，停止滚动，使用后，auto被设置为false
...
function CHIscroll(sJQcontainer, sJQkid, oControllers, oOptions, oCallbacks) {
	...
	function messure() {
		stop(); //先停止所有事件
		if (bMouseStop) {
			oJQcontainer.unbind("mouseover", stop);
			oJQcontainer.unbind("mouseleave", start);
		}
	...
	function run() {
		...
		iNextPosition = (cSign == '-') ? ((iMarginLength < piAllSize) ? ("-=" + piStep) : ("+=" + (piAllSize - piStep))) : ((iMarginLength > piAllSize - piContainerSize) ? ("+=" + piStep) : ("-=" + (piAllSize - piStep)));
		//if(p=='-') n=(ml < aS)?("-="+l):("+="+(aS-l));
...
/*{//反滚时，当可见移动层在新层（第二个层）上，正常进行
		...
		if (oCallbacks.move) oCallbacks.move(oJQcontainer, oJQkid);
	}
